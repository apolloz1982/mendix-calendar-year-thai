...
define([
    ...
    return declare("jQueryUIDateTimePicker.widget.jQueryUIDateTimePicker", [
        ...
        update: function update(obj, callback) {
			...
			logger.debug("dateAttribute -> " + this.dateAttribute);
			logger.debug(this.dateAttribute + " -> " + obj.get(this.dateAttribute));
			var d = new Date(obj.get(this.dateAttribute));
			logger.debug("date -> " + d);
			logger.debug("year -> " + $.datepicker.formatDate('yy', d));
			logger.debug("month -> " + $.datepicker.formatDate('mm', d));
			logger.debug("day -> " + $.datepicker.formatDate('dd', d));
			logger.debug("time -> " + d.getTime());
			var dateParseModify = $.datepicker.parseDate("ddmmyy",  $.datepicker.formatDate('dd', d) 
															+ $.datepicker.formatDate('mm', d) 
															+ ( parseInt($.datepicker.formatDate('yy', d) ) - 543) );
			logger.debug("dateParseModify time -> " + dateParseModify.getTime());
